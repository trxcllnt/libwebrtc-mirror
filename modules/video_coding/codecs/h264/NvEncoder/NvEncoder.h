...
#ifndef MODULES_VIDEO_CODING_CODECS_H264_NVENCODER_NVENCODER_H_
#define MODULES_VIDEO_CODING_CODECS_H264_NVENCODER_NVENCODER_H_
...
#define NVENC_THROW_ERROR(errorStr, errorCode)                                \
  std::cerr << __FUNCTION__ << " : " << errorStr << " at " << __FILE__ << ":" \
            << __LINE__ << std::endl;                                         \
  return errorCode;

#define NVENC_API_CALL(expr)                             \
  do {                                                   \
    auto const status = (expr);                          \
    if (status != NV_ENC_SUCCESS) {                      \
      std::ostringstream errorLog;                       \
      errorLog << #expr << " returned error " << status; \
      NVENC_THROW_ERROR(errorLog.str(), status);         \
    }                                                    \
  } while (0)
...
class NvEncoder {
 public:
   ...
   */
  int32_t Initialize(NV_ENC_DEVICE_TYPE eDeviceType,
                     void* pDevice,
                     uint32_t nWidth,
                     uint32_t nHeight,
   ...
   *  starting to encode any frames.
   */
  int32_t CreateEncoder(const NV_ENC_INITIALIZE_PARAMS* pEncodeParams);
   ...
   */
  int32_t Reconfigure(const NV_ENC_RECONFIGURE_PARAMS* pReconfigureParams);
   ...
   */
  int32_t EncodeFrame(std::vector<std::vector<uint8_t>>& vPacket,
   ...
   */
  int32_t EndEncode(std::vector<std::vector<uint8_t>>& vPacket);
   ...
   */
  int32_t CreateDefaultEncoderParams(
   ...
   */
  int32_t GetInitializeParams(NV_ENC_INITIALIZE_PARAMS* pInitializeParams);
   ...
   */
  int32_t RunMotionEstimation(std::vector<uint8_t>& mvData);
   ...
   */
  int32_t GetSequenceParams(std::vector<uint8_t>& seqParams);
 ...
 public:
   ...
   * formats.
   */
  static int32_t GetChromaSubPlaneOffsets(
  ...
  const char* GetLastErrorString() const {
    return m_nvenc.nvEncGetLastErrorString(m_hEncoder);
  }
 ...
 protected:
   ...
   */
  int32_t RegisterInputResources(std::vector<void*> inputframes,
  ...
  void* GetCompletionEvent(uint32_t eventIdx) {
    return (m_vpCompletionEvent.size() == static_cast<size_t>(m_nEncoderBuffer))
   ...
   *         NVENC hardware.
   */
  int32_t DoEncode(NV_ENC_INPUT_PTR inputBuffer,
   ...
   */
  int32_t DoMotionEstimation(NV_ENC_INPUT_PTR inputBuffer,
   ...
   */
  int32_t MapResources(uint32_t bfrIdx);
  ...
  */
   ...
   */
  int32_t GetEncodedPacket(std::vector<NV_ENC_OUTPUT_PTR>& vOutputBuffer,
   ...
   */
  int32_t InitializeBitstreamBuffer();
 ...
 private:
   ...
   */
  virtual int32_t AllocateInputBuffers(int32_t numInputBuffers) = 0;
