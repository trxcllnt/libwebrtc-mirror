...
int32_t NvEncoder::Initialize(NV_ENC_DEVICE_TYPE eDeviceType,
                              void* pDevice,
                              uint32_t nWidth,
                              uint32_t nHeight,
...
int32_t NvEncoder::LoadNvEncApi() {
  uint32_t version = 0;
...
int32_t NvEncoder::CreateDefaultEncoderParams(
...
int32_t NvEncoder::CreateEncoder(
    const NV_ENC_INITIALIZE_PARAMS* pEncoderParams) {
  if (!m_hEncoder) {
    NVENC_THROW_ERROR("Encoder Initialization failed",
                      NV_ENC_ERR_NO_ENCODE_DEVICE);
  }
  ...
  for (uint32_t i = 0; i < m_vpCompletionEvent.size(); i++) {
    ...
    NVENC_API_CALL(m_nvenc.nvEncRegisterAsyncEvent(m_hEncoder, &eventParams));
  }
#endif
  ...
  if (m_bMotionEstimationOnly) {
    ...
    if (!m_bOutputInVideoMemory) {
      NVENC_API_CALL(InitializeMVOutputBuffer());
    }
  } else {
    if (!m_bOutputInVideoMemory) {
      m_vBitstreamOutputBuffer.resize(m_nEncoderBuffer, nullptr);
      NVENC_API_CALL(InitializeBitstreamBuffer());
    }
  }

  return AllocateInputBuffers(m_nEncoderBuffer);
}
...
int32_t NvEncoder::MapResources(uint32_t bfrIdx) {
...
int32_t NvEncoder::EncodeFrame(std::vector<std::vector<uint8_t>>& vPacket,
  ...
  NVENC_API_CALL(MapResources(bfrIdx));

  int32_t nvStatus = DoEncode(m_vMappedInputBuffers[bfrIdx],
...
int32_t NvEncoder::RunMotionEstimation(std::vector<uint8_t>& mvData) {
  if (!m_hEncoder) {
    NVENC_THROW_ERROR("Encoder Initialization failed",
                      NV_ENC_ERR_NO_ENCODE_DEVICE);
  }
  ...
  int32_t nvStatus = DoMotionEstimation(m_vMappedInputBuffers[bfrIdx],
...
int32_t NvEncoder::GetSequenceParams(std::vector<uint8_t>& seqParams) {
...
int32_t NvEncoder::DoEncode(NV_ENC_INPUT_PTR inputBuffer,
  ...
  return m_nvenc.nvEncEncodePicture(m_hEncoder, &picParams);
}

int32_t NvEncoder::SendEOS() {
...
int32_t NvEncoder::EndEncode(std::vector<std::vector<uint8_t>>& vPacket) {
  vPacket.clear();
  if (!IsHWEncoderInitialized()) {
...
int32_t NvEncoder::GetEncodedPacket(
    std::vector<NV_ENC_OUTPUT_PTR>& vOutputBuffer,
    std::vector<std::vector<uint8_t>>& vPacket,
    bool bOutputDelay) {
  unsigned i = 0;
...
int32_t NvEncoder::RegisterResource(NV_ENC_REGISTER_RESOURCE& registerResource,
                                    void* pBuffer,
...
int32_t NvEncoder::RegisterInputResources(
  ...
  for (uint32_t i = 0; i < inputframes.size(); ++i) {
    ...
    NVENC_API_CALL(RegisterResource(registerResource, inputframes[i],
                                    eResourceType, width, height, pitch,
    ...
    NVENC_API_CALL(NvEncoder::GetChromaSubPlaneOffsets(bufferFormat, pitch,
                                                       height, _chromaOffsets));
...
int32_t NvEncoder::FlushEncoder() {
...
int32_t NvEncoder::GetChromaSubPlaneOffsets(
    const NV_ENC_BUFFER_FORMAT bufferFormat,
...
int32_t NvEncoder::GetInitializeParams(
...
int32_t NvEncoder::InitializeBitstreamBuffer() {
...
int32_t NvEncoder::InitializeMVOutputBuffer() {
  for (int i = 0; i < m_nEncoderBuffer; i++) {
...
int32_t NvEncoder::DestroyMVOutputBuffer() {
...
int32_t NvEncoder::DoMotionEstimation(NV_ENC_INPUT_PTR inputBuffer,
  ...
  return m_nvenc.nvEncRunMotionEstimationOnly(m_hEncoder, &meParams);
}
