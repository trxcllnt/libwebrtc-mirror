...
TEST(H264NvEncoderImplTest, CanInitializeWithDefaultParameters) {
  H264NvEncoderImpl encoder(cricket::VideoCodec("H264"));
...
TEST(H264NvEncoderImplTest, CanInitializeWithNonInterleavedModeExplicitly) {
  cricket::VideoCodec codec("H264");
  codec.SetParam(cricket::kH264FmtpPacketizationMode, "1");
  H264NvEncoderImpl encoder(codec);
  VideoCodec codec_settings;
  SetDefaultSettings(&codec_settings);
  EXPECT_EQ(WEBRTC_VIDEO_CODEC_OK,
            encoder.InitEncode(&codec_settings, kSettings));
...
TEST(H264NvEncoderImplTest, CanInitializeWithSingleNalUnitModeExplicitly) {
  cricket::VideoCodec codec("H264");
...
TEST(H264NvEncoderImplTest, CanInitializeWithRemovedParameter) {
  cricket::VideoCodec codec("H264");
